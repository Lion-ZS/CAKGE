...
class DataLoader:
    ...
    def get_edges(self, q_sub, q_rel, q_obj, mode='transductive'):
        ...
        if mode == 'transductive':  
            ...
            if q_obj is not None:  
                edges = edges.t()  
                remove_edges = torch.cat([q_sub.unsqueeze(-1), q_rel.unsqueeze(-1), q_obj.unsqueeze(-1)], dim=-1).t()  
    ...
    def get_next_layer_nodes_edges(self, nodes, n_ent, M_sub, np_filter_edges):
        ...
        selected_edges = torch.tensor(np_selected_edges, dtype=torch.long, device=device)  
        next_layer_nodes = torch.unique(selected_edges[:, [0, -1]], dim=0, sorted=False)  
