"""
full CAKGE implementation for inductive setting
"""
import torch
...
class G_GAT_Layer(torch.nn.Module):
    ...
    def __init__(self, in_dim, out_dim, attn_dim, n_rel, act=lambda x: x):
        ...
        # ---------- �� MoE ����ר�� ----------
        # K �� element?wise scale ����
        self.expert_scale = nn.Parameter(torch.empty(self.n_expert, in_dim))
        nn.init.xavier_uniform_(self.expert_scale)   # ��
        # ����ר��Ȩ�� (��) �������֪��
        self.gate_fc = nn.Sequential(
            nn.Linear(in_dim * 3, attn_dim, bias=False),
            nn.ReLU(),
            nn.Linear(attn_dim, self.n_expert, bias=False)
        )     
        
        

    def featurewise_ot(self, hidden, hidden_new, alpha=0.2):
        n1, d = hidden.shape
        n2 = hidden_new.shape[0]
        idx = torch.randperm(n2)[:n1]  
        hidden_new_sampled = hidden_new[idx].clone()  # 创建副本避免 inplace 修改
        for j in range(d):
            h_src_j, _ = torch.sort(hidden[:, j])
            h_tgt_j, idx_tgt = torch.sort(hidden_new_sampled[:, j])
            update = torch.zeros_like(hidden_new_sampled[:, j])
            update[idx_tgt] = alpha * (h_src_j - h_tgt_j)
            # 使用非 inplace 操作
            hidden_new_sampled = hidden_new_sampled.clone()
            hidden_new_sampled[:, j] = hidden_new_sampled[:, j] + update
        # 使用非 inplace 操作
        hidden_new = hidden_new.clone()
        hidden_new[idx] = hidden_new_sampled
        return hidden_new
    ...
    def forward(self, q_sub, q_rel, hidden, edges, n_node, old_nodes_new_idx):
        ...
        ##new 
        gate_inp = torch.cat([h_s, h_r, h_qr], dim=-1)      # (E, 3d)
        gate_logits = self.gate_fc(gate_inp)                # (E, K)
        K_weight = torch.softmax(gate_logits, dim=-1)       # ��_{(e,r,x)|u,q}^{?,i}
        # ---------- �� ? ��ר��������Ϣ ----------
        # (E, 1, d) �� (1, K, d) -> (E, K, d)
        exp_msg = unique_message.unsqueeze(1) * self.expert_scale.unsqueeze(0)
        # ---------- �� ? ��Ȩ���ں� ----------
        unique_message = (K_weight.unsqueeze(-1) * exp_msg).sum(dim=1)  # (E, d)
        ...
        hidden_new=self.featurewise_ot(hidden, hidden_new)
