...
class BaseModel(nn.Module):
    def __init__(self, args, kg) -> None:
        ...
        self.alpha_caf = 0.1
        self.n_expert = 4
        expert_scale_init = torch.empty(self.n_expert, self.args.emb_dim, device=self.args.device, dtype=torch.float64)
        nn.init.xavier_uniform_(expert_scale_init)
        self.expert_scale = nn.Parameter(expert_scale_init)
        self.gate_fc = nn.Sequential(
            nn.Linear(self.args.emb_dim * 3, self.args.emb_dim, bias=False),
            nn.ReLU(),
            nn.Linear(self.args.emb_dim, self.n_expert, bias=False)
        ).to(self.args.device).double()

    def context_aware_fusion(self, h, r, t):
        gate_inp = torch.cat([h, r, t], dim=-1)
        
        gate_logits = self.gate_fc(gate_inp)
        pi = torch.softmax(gate_logits, dim=-1)  
        
        h_exp = h.unsqueeze(1) * self.expert_scale.unsqueeze(0)
        h_moe = (pi.unsqueeze(-1) * h_exp).sum(dim=1)  
        
        t_exp = t.unsqueeze(1) * self.expert_scale.unsqueeze(0)
        t_moe = (pi.unsqueeze(-1) * t_exp).sum(dim=1)
    ...
    def score_fun(self, h, r, t):
        if self.alpha_caf > 0:
            h_moe, t_moe = self.context_aware_fusion(h, r, t)
            h = h + self.alpha_caf * h_moe
            t = t + self.alpha_caf * t_moe
