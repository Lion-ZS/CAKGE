...
class G_GAT_Layer(torch.nn.Module):
    ...
    def featurewise_ot(self, hidden, hidden_new, alpha=0.1):
        n1, d = hidden.shape
        n2 = hidden_new.shape[0]
        idx = torch.randperm(n2)[:n1]  
        hidden_new_sampled = hidden_new[idx]  
        for j in range(d):
    ...
    def forward(self, q_sub, q_rel, hidden, edges, n_node, old_nodes_new_idx):
        ...
        gate_inp = torch.cat([h_s, h_r, h_qr], dim=-1)     
        gate_logits = self.gate_fc(gate_inp)                
        K_weight = torch.softmax(gate_logits, dim=-1)      
        exp_msg = unique_message.unsqueeze(1) * self.expert_scale.unsqueeze(0)
        unique_message = (K_weight.unsqueeze(-1) * exp_msg).sum(dim=1) 
